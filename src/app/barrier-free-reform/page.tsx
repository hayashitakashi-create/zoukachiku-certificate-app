...
function BarrierFreeReformContent() {
  ...
  const onSubmit = async (data: BarrierFreeFormData) => {
    if (!certificateId) {
      ...
      localStorage.setItem('calc_result_barrierFree', String(totalAmount));
      if (data.subsidyAmount > 0) {
        localStorage.setItem('calc_subsidy_barrierFree', String(data.subsidyAmount));
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-stone-50 via-amber-50/30 to-orange-50/30">
      <header className="bg-white/90 border-b border-stone-200 sticky top-0 z-50 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-5 flex items-center justify-between">
          <h1 className="text-lg sm:text-xl font-bold bg-gradient-to-r from-stone-800 to-amber-900 bg-clip-text text-transparent">バリアフリー改修工事</h1>
          <Link
