...
function LongTermHousingContent() {
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<LongTermHousingFormData>({
    resolver: zodResolver(longTermHousingFormSchema),
    defaultValues: {
    ...
    <div className="min-h-screen bg-gradient-to-br from-stone-50 via-amber-50/30 to-orange-50/30">
      <header className="bg-white/90 border-b border-stone-200 sticky top-0 z-50 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-5 flex items-center justify-between">
          <h1 className="text-lg sm:text-xl font-bold bg-gradient-to-r from-stone-800 to-amber-900 bg-clip-text text-transparent">長期優良住宅化改修工事</h1>
          <Link
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl sm:rounded-3xl shadow-xl shadow-stone-200/50 border border-stone-200 p-4 sm:p-6 md:p-8 mb-6">
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            {/* 長期優良住宅認定チェックボックス */}
            <div className="mb-6 bg-gradient-to-br from-amber-50 to-stone-50 rounded-2xl p-4 border border-amber-100">
              <label className="flex items-center">
                <input
                  type="checkbox"
                  {...register('isExcellentHousing')}
                ...
                <span className="ml-2 text-sm font-semibold text-stone-700">
                  長期優良住宅の認定を取得済み（控除対象額の上限: 500万円）
                </span>
              </label>
              <p className="mt-2 text-xs text-stone-500 ml-6">
                ※ 認定未取得の場合は上限250万円となります
              </p>
            </div>
